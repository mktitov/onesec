 ...
 */
public class JoinToConferenceAction extends AbstractAction {
    public final static String NAME = "Join to conference";
...
//    public final static String CONFERENCE_ACCESS_CODE_BINDING = "conferenceAccessCode";
    ...
    protected ActionExecuted processExecuteMessage(Execute message) throws Exception {
        ...
        final ConversationScenarioState conversationState = conversation.getConversationScenarioState();
        final Bindings bindings = conversationState.getBindings();
...
//        String conferenceAccessCode = (String) bindings.get(CONFERENCE_ACCESS_CODE_BINDING);
        ConferenceSessionState state = (ConferenceSessionState) bindings.get(CONFERENCE_STATE_BINDING);
        if (state==null) {
            if (getLogger().isDebugEnabled())
                getLogger().debug("Connectiong to conference...");
